...
from micro.modulos.batchadmin.forms import ActionForm, checkbox, CHECKBOX_NAME
from micro.modulos.batchadmin.util import model_format_dict, model_ngettext, get_changelist
...
class BatchModelAdmin(admin.ModelAdmin):
    ...
    batchadmin_checkbox.short_description = mark_safe('marcar')
    ...
    def delete_selected(self, request, changelist):
        if self.has_delete_permission(request):
            ...
            if n:
                ...
                objects.delete()
                self.message_user(request, u"%d %s Deletado com sucesso." % (
                    n, model_ngettext(self.opts, n)
                ))
    delete_selected.short_description = u"Deletar Selecionados"
